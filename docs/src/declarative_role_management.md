...
To write the three files out, set the `umask` first: libpq refuses a private key
file that is readable by the group or by anyone else, and reports it as
`private key file "tls.key" has group or world access`, which reads like a
problem with the certificate rather than with its permissions.
...
An application that consumes the Secret as a mounted volume has the same
constraint, and meets it with `defaultMode: 0600` on the volume. See
...
:::important
Every renewal replaces both `tls.crt` and `tls.key` in the Secret. This is on
purpose, and it sets role certificates apart from the other certificates the
operator manages: a cluster's server certificate and the `streaming_replica`
one are re-signed onto the key they already had, while a role certificate gets
a new key each time. The Secret only ever holds one key pair, so there is no
window in which the old and the new certificate are both usable. Sessions that
are already established are not
