...
func (r *DatabaseRoleReconciler) ensureOwnedCertSecretUpToDate(
	...
	// The cases are ordered so that the ones reading the parsed certificate are
	// reached only once it is known to be readable.
	...
	switch {
	case err != nil:
		// A parse or chain-verification error means the cert is corrupt; treat
		// it as a re-issue trigger rather than error-looping.
		reason = fmt.Sprintf("the certificate is unreadable: %v", err)
	case !signedByCurrentCA:
		reason = "the client CA changed"
	case keyErr != nil:
		// A Secret whose key was deleted, or replaced with one belonging to
		// another certificate, is useless for client authentication, so it is
		// re-issued rather than left in place until its renewal window.
	...
	case leaf.NotAfter.Sub(leaf.NotBefore) != lifetime.Truncate(time.Second):
		...
		// either because the role now sets a different duration or because the
		// operator-wide default did.
	...
	contextLogger.Info("re-issuing client certificate", "secret", secretKey.Name, "reason", reason)
...
// clientCertSecretKey returns the key of the Secret holding the role's
...
// could not be issued, or why an existing Secret was left alone, and logs the
// same explanation. Any expiration already observed is kept: a certificate
// signed before the problem appeared is still usable, and its expiry is what
// tells the administrator for how long.
...
// clientCertDuration returns the lifetime the role's client certificate is
// issued with, falling back to the operator-wide default when the role does
// not set one.
